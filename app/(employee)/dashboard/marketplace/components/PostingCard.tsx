...
import type { JobView } from "@/lib/queries/marketplace";
 ...
 *  allowed (PRD 5.1.9). */
export default function PostingCard({ job }: { job: JobView }) {
  ...
  return (
    <article className="rounded-m border border-line bg-surface p-4">
      <div className="flex items-baseline justify-between gap-4">
        <Eyebrow>{job.departmentName}</Eyebrow>
      ...
      <div className="mt-2 flex items-center justify-between gap-4">
        <p className="text-sm text-secondary">{job.accountName}</p>
        {job.status !== "OPEN" ? (
